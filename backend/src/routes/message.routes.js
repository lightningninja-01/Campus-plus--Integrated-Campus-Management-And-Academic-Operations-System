...
router.get("/:groupId", authMiddleware, async (req, res) => {
  try {
    ...
    if (!isMember) return res.status(403).json({ message: "Join the group to view messages" });
...
router.post("/:groupId", authMiddleware, async (req, res) => {
  try {
    ...
    const isMember = group.members.map(String).includes(String(req.user.id));
    if (!isMember) return res.status(403).json({ message: "Join the group to send messages" });
